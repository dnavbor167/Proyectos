...
$(function(){
    if(localStorage.token)
    {
        ...
        {
            localStorage.clear();
            cargar_vista_login("Su tiempo de sesión a expirado");
        }
    }
    else
        cargar_vista_login("");
...
{
    $('#errores').html("");
    let html_principal="<div id='saludo'>Bienvenido <strong>"+datos_usu_log["usuario"]+"</strong> - <button onclick='cerrar_sesion();' class='enlace'>Salir</button></div>";
    html_principal+="<div id='respuestas'></div><div id='productos'></div>";
    $('#principal').html(html_principal);

    if(datos_usu_log["tipo"]=="admin")
    {
        //función de la librería index
        obtener_productos();
    }
